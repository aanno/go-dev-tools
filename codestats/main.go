...
func getAuthorBlame(repo *git.Repository, path string) string {
    ...
    // Use git blame via go-git
    blame, err := repo.Blame(&git.BlameOptions{
        Hash: head.Hash(),
        Path: path,
    })
    if err != nil {
        log.Printf("Warning: blame failed for %s: %v (falling back to log)", path, err)
        return getAuthorFromLog(repo, path, "", "")
    }
    
    // Count authors from blame results
...
func getAuthorRange(repo *git.Repository, path string, fromHash, toHash string) string {
    ...
    if err != nil {
        log.Printf("Invalid from commit %s: %v", fromHash, err)
        return "unknown"
    }
    
    toCommit, err := repo.CommitObject(plumbing.NewHash(toHash))
    if err != nil {
        log.Printf("Invalid to commit %s: %v", toHash, err)
        return "unknown"
    }
    
    return getAuthorFromLog(repo, path, fromCommit.Hash.String(), toCommit.Hash.String())
}
...
func getAuthorFromLog(repo *git.Repository, path string, fromHash, toHash string) string {
    ...
    if fromHash != "" && toHash != "" {
        ...
        logOptions.From = to
        // Note: go-git doesn't have a direct "until" parameter
        // We'll iterate and stop when we reach fromHash
    }
    ...
    err = iter.ForEach(func(c *object.Commit) error {
        // Stop if we've passed the from commit
